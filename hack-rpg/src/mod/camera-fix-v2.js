import 'hackforplay/rpg-kit-camera';
...
var Camera = enchant.Class.create(enchant.Node, {
	initialize: function() {
	...
	clampRect: function(rect) {
		...
		if (!_d_x) rect.x = Math.clamp(rect.x, 0.0, Hack.map.width - w);
		if (!_d_y) rect.y = Math.clamp(rect.y, 0.0, Hack.map.height - h);
	...
	// 廃止
	_deprecated$clampScale: true,
	_deprecated$clampScaleRect: function(rect) {
		
		var size = this.getVisionSize();
		...
		if (rect.widht > Hack.map.width) {
			ox =  rect.width/Hack.map.width  ;
		}
		if (rect.height > Hack.map.height) {
			oy = rect.height/Hack.map.height  ;
		}
		...
		if (o !== null) {
			...
			rect.width = rect.width * o;
			rect.height = rect.height * o;
