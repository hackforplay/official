...
class Laser extends RPGObject {
    ...
    update() {
        ...
        const newLine = new Line(
            ...
            lastReflectionPoint.point.add(lastReflectionPoint.direction.normalize().scale(this.position - lastReflectionPoint.position))
        );
        ...
        (function check(newLine) {
            ...
            const checkLines = reflectionLines.filter((line) => {
                return !lastReflectionPoint.targetLines.some((targetLine) => Line.equal(line, targetLine));
            });
            ...
            if (Vector2.equal(collidedPoint, lastReflectionPoint.point)) {
                ...
                lastReflectionPoint.direction = lastReflectionPoint.direction.rotate(Math.PI / 2);
            ...
            if (currentLength <= this.speed) {
                ...
                const newLineDirection = newLine.end.subtract(newLine.start).normalize();
                if (reflectionCount) {
                    this.dispatchEvent(new Event('reflect', {
                ...
                this.points.push({
                    ...
                    // 最後にぶつかった点との距離
                    position: lastReflectionPoint.position + collidedPoint.subtract(lastReflectionPoint.point).magnitude()
                });
    ...
    render() {
        ...
        if (startPosition === endPosition) {
            ...
            // まだ反射してない場合は原点から計算する
            points.push(this.points[0].point.add(this.points[0].direction.normalize().scale(startPosition)));
            points.push(this.points[0].point.add(this.points[0].direction.normalize().scale(endPosition)));
        }
        else {
            // 反射点を繋ぐ
            if (startPosition === 0) {
            ...
            for (let i = 0; i < this.points.length - 1; ++i) {
                ...
                if (p1.position <= startPosition && p2.position > startPosition) {
                    points.push(p1.point.add(p1.direction.normalize().scale(startPosition - p1.position)))
                    isStart = true;
                }
                if (p1.position <= endPosition && p2.position > endPosition) {
                    points.push(p1.point.add(p1.direction.normalize().scale(endPosition - p1.position)))
                    isEnd = true;
                    break;
                }
            ...
            const lastReflectionPoint = this.points[this.points.length - 1];
            if (!isStart) {
                points.push(lastReflectionPoint.point.add(lastReflectionPoint.direction.scale(startPosition - lastReflectionPoint.position)))
            }
            if (!isEnd) {
                points.push(lastReflectionPoint.point.add(lastReflectionPoint.direction.scale(endPosition - lastReflectionPoint.position)))
            }
        }
